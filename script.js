...
function chargerOuvriers() {
  ...
  console.warn("⚠️ window.OUVRIERS non trouvé");
...
function remplirSelectOuvriers(list) {
  if (!selectOuvrier) {
    console.error("❌ select #ouvrierSelect introuvable");
    return;
  }
...
selectOuvrier?.addEventListener("change", () => {
  ...
  inputNaissance.value = normalizeDate(o.naissance);
...
function buildPdfWithJsPDF(d) {
  ...
  tableHeader(["Critère", "Appréciation"], [430, 200]);
  d.evaluation.forEach(row => {
